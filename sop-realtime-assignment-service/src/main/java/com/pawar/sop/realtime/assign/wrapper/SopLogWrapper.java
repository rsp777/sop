...
import com.pawar.sop.realtime.assign.config.SopLogServiceConfiguration;
...
//	}
	...
	public String createBatch(String actionType,String batchType) throws ClientProtocolException, IOException {
		...
		ResponseEntity<String> response = (ResponseEntity<String>) httpService.restCall(null,url, HttpMethod.POST,
				actionType, queryParams);
		batchId = response.getBody();
		logger.info("Created New Batch {} with actionType : {}", batchId, actionType);
		return batchId;
	}
	...
	public void updateBatchStatus(String batchId, int batchStatus) throws ClientProtocolException, IOException {
		...
		ResponseEntity<String> response = (ResponseEntity<String>) httpService.restCall(null,url, HttpMethod.PUT, batchDto,
				queryParams);
		batchId = response.getBody();
